...
get_rna_matrix2 <- function(gene_tumor, gene_normal, output)
{
	...
	gene_exprs <- cbind(gene_tumor[overlap_gene,],gene_normal[overlap_gene,])
	gene_exprs <- as.matrix(gene_exprs)
	copykat_res2 <- copykat2(rawmat=gene_exprs, id.type="S", cell.line = "yes", ngene.chr=5, win.size=25, KS.cut=0.1, sam.name=paste0(output,"results"), distance="euclidean", norm.cell.names=colnames(gene_normal), n.cores=16)
...
output <- './Results/Tian/copykat/10x_3cl_310/results/'
...
gene_tumor <- gene_exprs[,as.character(sample_anno[sample_anno[,2]=='Tumor',1])]
gene_normal <- gene_exprs[,as.character(sample_anno[sample_anno[,2]=='Normal',1])]
results <- get_rna_matrix(gene_tumor, gene_normal, output)


## test for scicnv normalized data ##
...
input <- './Results/Tian/infercnv/10x_3cl_310/'
output <- './Results/Tian/copykat/10x_3cl_310/results_scicnv_norm/'
dir.create(output,recursive=T)

gene_exprs <- read.table(paste0(input,'/norm_counts_scicnv.txt.gz'),header=T,sep='\t')
sample_anno <- read.table(paste0(input,'/sample_anno.txt.gz'))
gene_tumor <- gene_exprs[,as.character(sample_anno[sample_anno[,2]=='Tumor',1])]
gene_normal <- gene_exprs[,as.character(sample_anno[sample_anno[,2]=='Normal',1])]
results <- get_rna_matrix(gene_tumor, gene_normal, output)
...
input <- './Results/Tian/infercnv/10x_3cl_310/'
#output <- './Results/Tian/copykat/10x_3cl_310/results_decon_nocelltype/'
output <- './Results/Tian/copykat/10x_3cl_310/results_decon_celltype/'
dir.create(output,recursive=T)

gene_exprs <- read.table(paste0(input,'/decon_counts_celltype.txt.gz'),header=T,sep='\t')
sample_anno <- read.table(paste0(input,'/decon_sample_anno_celltype.txt.gz'))
gene_tumor <- gene_exprs[,as.character(sample_anno[sample_anno[,2]=='Tumor',1])]
gene_normal <- gene_exprs[,as.character(sample_anno[sample_anno[,2]=='Normal',1])]
results <- get_rna_matrix(gene_tumor, gene_normal, output)
...
output <- './Results/Tian/copykat/10x_3cl_220/results/'
dir.create(output,recursive=T)
...
input <- './Results/Tian/infercnv/10x_3cl_220/'
output <- './Results/Tian/copykat/10x_3cl_220/results_scicnv_norm/'
dir.create(output,recursive=T)
...
output <- './Results/Tian/copykat/10x_5cl_310/results/'
dir.create(output,recursive=T)
...
output <- './Results/Tian/copykat/10x_5cl_201/results/'
dir.create(output,recursive=T)
...
output <- './Results/Tian/copykat/celseq2/results/'
dir.create(output,recursive=T)
...
output <- './Results/Tian/copykat/dropseq/results/'
dir.create(output,recursive=T)
