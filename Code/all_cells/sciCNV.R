...
input2 <- './Anno/'
output <- './Results/10x/all_cell/scicnv/v2/'
dir.create(output,recursive=T)

sample_anno <- read.table(paste0(input,'/sample_anno_v2.txt.gz'))
...
gene_exprs <- read.table(paste0(input,'/raw_counts_v2.txt.gz'),header=T,sep='\t')
...
input2 <- './Anno/'
output <- './Results/c1_fda/all_cell/scicnv/v2/'
dir.create(output,recursive=T)
...
input2 <- './Anno/'
output <- './Results/c1_llu/all_cell/scicnv/v2/'
dir.create(output,recursive=T)
...
input2 <- './Anno/'
output <- './Results/WaferGen/all_cell/scicnv/v2/'
dir.create(output,recursive=T)
...
input2 <- './Anno/'
output <- './Results/bulk/scicnv/v3/'
dir.create(output,recursive=T)

sample_anno <- read.table(paste0(input,'/sample_anno_v3.txt.gz'))
gene_anno <- read.csv('./Anno/10x_gene_anno.csv')
gene_anno2 <- read.csv('./Code/sciCNV/Dataset/10XGenomics_gen_pos_GRCh38-1.2.0.txt',header=T,sep='\t')
test_num <- as.numeric(table(sample_anno[,2])[2])

gene_exprs <- read.table(paste0(input,'/raw_counts_v3.txt.gz'),header=T,sep='\t')
index <- match(rownames(gene_exprs),gene_anno[,1])
gene_exprs <- gene_exprs[!is.na(index),]
rownames(gene_exprs) <- gene_anno[index[!is.na(index)],2]
index <- match(rownames(gene_exprs),gene_anno2[,1])
