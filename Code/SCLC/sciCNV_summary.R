...
da_cnv <- read.csv(paste0(input,'cnv_genes.csv'),row.names=1)
gene_id <- rownames(da_cnv)
...
input <- paste0('./Results/SCLC/scicnv/',status,'/')

sens_spec_all <- sens_spec_loss <- sens_spec_gain <- c()
for(i in 1:length(input))
{
	if(i == 1 )
	{temp <- get_sens_spec_scicnv(input[i], gene_anno, cnv_primary_loss, cnv_primary_gain, 'v1')}
	if(i == 2)
	{temp <- get_sens_spec_scicnv(input[i], gene_anno, cnv_relapse_loss, cnv_relapse_gain, 'v1')}
...
output <- './Results/SCLC/scicnv/summary/'
dir.create(output,recursive=T)
